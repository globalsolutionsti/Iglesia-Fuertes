...
function renderWelcomeActionModal_() {
  const activeModal = state.ui.welcomeModal;

  if (!activeModal) {
    return "";
  }

  if (activeModal.kind === "prospect-feedback") {
    const feedbackState = getWelcomeProspectFeedbackModalState_();
    ...
    const hasTargets = leaderTargets.length > 0;

    return `
      <div class="system-modal-backdrop">
        <section class="system-modal-card welcome-prospect-modal" role="dialog" aria-modal="true" aria-labelledby="welcome-feedback-modal-title">
          <div class="panel-head">
            <div>
              <h2 id="welcome-feedback-modal-title">WhatsApp interno al líder</h2>
              <p>Registra la retroalimentación de Bienvenida en la bitácora y abre WhatsApp listo para contactar al liderazgo del grupo.</p>
            </div>
            <span class="pill dark">${escapeHtml(workflow.stage === "PENDING" ? "Pendiente por registrar" : workflow.label)}</span>
          </div>
          ...
          <div class="summary-stack welcome-prospect-modal-summary" style="margin-bottom: 16px;">
            <div class="summary-box welcome-prospect-summary-box">
              <span class="status-chip neutral">Prospecto</span>
            ...
            <div class="summary-box welcome-prospect-summary-box">
              ...
              <strong>${escapeHtml(group?.name || person.suggestedGroupName || "Sin grupo asignado")}</strong>
              <span>${escapeHtml(getWelcomeLeaderContactSummary_(person))}</span>
            </div>
            <div class="summary-box welcome-prospect-summary-box">
              <span class="status-chip neutral">Último aviso</span>
              <strong>${escapeHtml(formatDate(person.prospectAlertAt) || "Sin fecha")}</strong>
              <span>${escapeHtml(person.prospectAlertStatus || "Telegram pendiente o no registrado")}</span>
            </div>
            <div class="summary-box welcome-prospect-summary-box">
              <span class="status-chip ${escapeHtml(hasTargets ? "success" : "warning")}">Canal interno</span>
              <strong>${escapeHtml(hasTargets ? "WhatsApp disponible" : "Falta WhatsApp líder")}</strong>
              <span>${escapeHtml(hasTargets
                ? (selectedTarget ? `${selectedTarget.name || "Líder"}${selectedTarget.phone ? ` · ${selectedTarget.phone}` : ""}` : "Selecciona destinatario")
                : "Configura al menos un teléfono de líder en Catálogos para usar este canal.")}</span>
            </div>
          </div>

          <form id="welcome-prospect-feedback-form">
            <input type="hidden" name="personId" value="${escapeHtml(person.id || "")}">
            <div class="field-grid two">
              <div class="field">
                <label for="welcome-prospect-feedback-date">Fecha de contacto interno</label>
                <input id="welcome-prospect-feedback-date" name="actionDate" type="date" value="${escapeHtml(actionDate)}">
              </div>
              <div class="field">
                <label for="welcome-prospect-feedback-target">Enviar a</label>
                <select id="welcome-prospect-feedback-target" name="targetIndex" ${hasTargets ? "" : "disabled"}>
                  ${leaderTargets.length
                    ? leaderTargets.map((target, index) => `
                      <option value="${escapeHtml(String(index))}" ${index === selectedTargetIndex ? "selected" : ""}>
                        ${escapeHtml(target.name || `Líder ${index + 1}`)}${target.phone ? ` · ${escapeHtml(target.phone)}` : ""}
                      </option>
                    `).join("")
                    : `<option value="0" selected>Sin líderes con WhatsApp</option>`}
                </select>
              </div>
              <div class="field" style="grid-column: 1 / -1;">
                <label for="welcome-prospect-feedback-notes">Mensaje interno de Bienvenida</label>
                <textarea id="welcome-prospect-feedback-notes" name="notes" rows="4" placeholder="Ejemplo: El prospecto sí respondió, pero todavía no confirma si asistirá al grupo este viernes.">${escapeHtml(notes)}</textarea>
              </div>
            </div>

            <div class="summary-box welcome-prospect-fallback-box tone-neutral" style="margin-top: 18px;">
              <span class="status-chip neutral">Bitácora pastoral</span>
              <strong>Este contacto también quedará guardado</strong>
              <span>Se registra quién lo envió, la fecha y el contexto compartido entre Bienvenida y Grupos de Conexión.</span>
            </div>

            <div class="actions-row welcome-prospect-modal-actions" style="margin-top: 18px;">
              <button class="btn btn-primary" type="submit" ${hasTargets ? "" : "disabled"}>Guardar y abrir WhatsApp</button>
      ...
      <section class="system-modal-card welcome-prospect-modal" role="dialog" aria-modal="true" aria-labelledby="welcome-modal-title">
        <div class="panel-head">
          <div>
            ...
            <p>Confirma los datos de la persona, elige el grupo correcto y guarda el aviso silencioso al líder por Telegram.</p>
          </div>
...
function getWelcomeProspectFeedbackModalState_() {
  const modal = state.ui.welcomeModal;

  if (!modal || modal.kind !== "prospect-feedback") {
    return null;
  }
  ...
  const groupId = String(sourcePerson?.suggestedGroupId || sourcePerson?.assignedGroupId || "").trim();
  const previewPerson = buildWelcomePersonClientDto_({
    ...sourcePerson,
    suggestedGroupId: groupId,
    grupoSugerido: groupId
  ...
  const group = state.catalogs.groups.find((item) => String(item.id) === groupId) || null;
  const actionDate = String(modal.actionDate || formatDateForInput_(new Date()) || "").trim();
  const notes = modal.notes !== undefined
    ? String(modal.notes || "")
    : "";
  const leaderTargets = buildWelcomeProspectLeaderFeedbackWhatsappTargets_(previewPerson, group, actionDate, notes);
  const normalizedTargetIndex = leaderTargets.length
    ? Math.min(Math.max(Number(modal.targetIndex || 0), 0), leaderTargets.length - 1)
    : 0;
  ...
  return {
    ...
    notes,
    leaderTargets,
    selectedTargetIndex: normalizedTargetIndex,
    selectedTarget: leaderTargets[normalizedTargetIndex] || null
  };
}
...
function buildWelcomeProspectLeaderFeedbackWhatsappTargets_(person, group, actionDate, notes) {
  const groupId = String(group?.id || person?.suggestedGroupId || "").trim();
  const groupName = String(group?.name || person?.suggestedGroupName || "").trim();
  const contacts = buildWelcomeLeaderContactsFromGroup_(groupId);
  ...
  return contacts
    .map((contact) => {
      ...
      return {
        name: String(contact.name || "Líder").trim(),
        phone: String(contact.phone || "").trim(),
        url: `https://wa.me/${phone}?text=${encodeURIComponent(buildWelcomeProspectLeaderFeedbackWhatsappText_(person, groupName, actionDate, notes, contact.name))}`
      };
    })
    .filter(Boolean);
}

function buildWelcomeProspectLeaderFeedbackWhatsappText_(person, groupName, actionDate, notes, leaderName) {
  const lines = [
    `Hola ${String(leaderName || "líder").trim()},`,
    "",
    "Te escribimos desde Bienvenida para dar seguimiento interno a este prospecto de Grupos de Conexión.",
    "",
    `Nombre: ${person?.nombreCompleto || person?.nombre || "Sin nombre"}`,
    `Teléfono: ${person?.telefono || "Sin teléfono"}`,
    `Grupo de conexión sugerido: ${groupName || "Sin grupo"}`,
    `Fecha de retroalimentación: ${formatDate(actionDate) || actionDate || "Sin fecha"}`
  ];

  if (notes) {
    lines.push(`Contexto de Bienvenida: ${notes}`);
  }

  lines.push(
    "",
    "¿Nos apoyas confirmando si ya hubo contacto, cómo va el seguimiento y si sigue pendiente el registro en el grupo?"
  );

  return lines.join("\n");
}

function buildWelcomeProspectLeaderFeedbackRecordNote_(person, groupName, actionDate, leaderName, notes) {
  const summary = [
    `Bienvenida preparó WhatsApp interno para ${leaderName || "líder pendiente"}.`,
    `Grupo: ${groupName || "Sin grupo"}.`,
    `Fecha: ${formatDate(actionDate) || actionDate || "Sin fecha"}.`
  ];

  if (person?.telefono) {
    summary.push(`Teléfono del prospecto: ${person.telefono}.`);
  }

  summary.push("Solicitud: confirmar contacto y avance del registro en Grupos de Conexión.");

  if (notes) {
    summary.push(`Contexto enviado: ${notes}`);
  }
...
function getStudentPortalCurrentRecord_(portal, currentLevel) {
  ...
  if (currentLevel?.levelId) {
    const currentRecord = records.find((record) => String(record?.levelId || "") === String(currentLevel.levelId));
...
function getStudentPortalHeadlineCopy_(currentLevel, currentRecord, nextLevel) {
  const status = String(currentLevel?.status || "").trim().toUpperCase();
  const currentName = currentLevel?.levelName || "tu proceso";
  const nextName = nextLevel?.levelName || "el siguiente nivel";

  if (!currentLevel) {
    return "Tu portal se activará en cuanto quede registrado tu primer nivel dentro del proceso de formación.";
  }

  if (status === "ACREDITADO") {
    return nextLevel
      ? `Ya acreditaste ${currentName}. Ahora tu siguiente paso visible es ${nextName}.`
      : `Ya acreditaste ${currentName}. Tu ruta está al día y el siguiente nivel se habilitará cuando corresponda.`;
  }

  if (status === "NO_ACREDITADO") {
    return `Tu nivel actual requiere seguimiento. Revisa asistencia, evaluación y contacto con tu líder para definir el siguiente paso.`;
  }

  if (status === "DISPONIBLE") {
    return `Tu siguiente nivel ya está disponible. Tu líder o coordinador podrá confirmar tu inscripción para iniciar ${currentName}.`;
  }

  if (status === "BLOQUEADO") {
    return `Este nivel se abrirá automáticamente cuando acredites el nivel anterior dentro del proceso.`;
  }

  if (currentRecord?.paymentPending) {
    return `Ya iniciaste ${currentName}. Mantente al día con asistencia, evaluación y seguimiento administrativo de tu proceso.`;
  }

  return `Ya iniciaste ${currentName}. Aquí verás tu avance, tus evidencias y lo que sigue en tu ruta formativa.`;
}

function getStudentPortalPaymentSnapshot_(record) {
  const amount = String(record?.paymentAmount || "").trim();
  const dueDate = formatDate(record?.paymentDueDate || "") || "";
  const paidAt = formatDate(record?.paymentPaidAt || "") || "";
  const status = String(record?.paymentStatus || "").trim().toUpperCase();
  const alertLabel = String(record?.paymentAlertLabel || "").trim();

  if (!record || (!status && !amount && !dueDate && !paidAt)) {
    return {
      kind: "slate",
      title: "Sin control de pago",
      copy: "Este nivel no tiene un dato de pago visible o aún no fue capturado por el equipo."
    };
  }
  ...
  if (status === "PAGADO") {
    return {
      kind: "green",
      title: amount ? `Pagado ${amount}` : "Pago cubierto",
      copy: paidAt ? `Registrado el ${paidAt}.` : "El equipo ya marcó el pago como completado."
    };
  }
  ...
  if (alertLabel) {
    return {
      kind: "gold",
      title: amount ? `Pendiente ${amount}` : "Pago pendiente",
      copy: dueDate ? `${alertLabel}. Limite ${dueDate}.` : alertLabel
    };
  }

  return {
    kind: "slate",
    title: amount ? `Pendiente ${amount}` : "Pendiente",
    copy: dueDate ? `Fecha limite ${dueDate}.` : "Tu pago sigue pendiente de confirmación."
  };
}

function renderStudentPortalStatCard_(options) {
  return `
    <article class="student-portal-stat-card student-portal-stat-${escapeHtml(options?.tone || "slate")}">
      <span class="status-chip neutral">${escapeHtml(options?.eyebrow || "")}</span>
...
function renderStudentPortalProgressBar_(attendance) {
  const attended = Number(attendance?.attendedSessions || 0);
  const total = Number(attendance?.totalSessions || 0);
  const percent = total ? Math.max(0, Math.min(100, Math.round((attended / total) * 100))) : 0;

  return `
    <div class="student-progress-bar">
      <span class="student-progress-bar-fill" style="width:${escapeHtml(String(percent))}%"></span>
    </div>
  `;
}
...
function renderStudentPortalLevelCard_(level) {
  ...
  const examScore = String(level?.enrollment?.examScore || "").trim();
  const descriptor = status === "ACREDITADO"
    ? "Nivel aprobado y desbloqueo completado."
    : status === "EN_CURSO"
      ? "Este es tu nivel activo en este momento."
      : status === "NO_ACREDITADO"
        ? "Requiere seguimiento antes de continuar."
        : status === "DISPONIBLE"
          ? "Listo para inscribirse cuando el equipo lo confirme."
          : "Aun no se libera este nivel.";
  ...
  return `
    <article class="student-level-card student-level-${escapeHtml(String(level?.status || "").toLowerCase())}">
      ...
      <div class="student-level-card-metrics">
        <span>${escapeHtml(`${attendance.attendedSessions || 0}/${attendance.totalSessions || 0} asistencias`)}</span>
        <span>${escapeHtml(examScore ? `Examen ${examScore}` : "Examen pendiente")}</span>
      </div>
    </article>
  `;
}
  ...
  return `
    <article class="student-history-card">
      <div class="student-history-card-head">
        <div>
          <small>Movimiento ${escapeHtml(String(index + 1))}</small>
          <strong>${escapeHtml(title)}</strong>
        </div>
        ${renderWorkflowStatusPill_(record?.status || "SIN_PROCESO")}
      </div>
...
function renderWelcomeFollowupView_() {
  ...
  const scopeMeta = ({
    ALL: {
      ...
      copy: "Aquí conviven los nuevos por atender y los casos que ya van caminando entre Bienvenida y Grupos de Conexión."
    },
    SIN_SEGUIMIENTO: {
      title: "Nuevos sin seguimiento",
      copy: "Aquí ves únicamente a quienes siguen sin primer seguimiento pastoral registrado."
    },
    CON_SEGUIMIENTO: {
      title: "Nuevos con seguimiento",
      copy: "Aquí aparecen las personas que ya tienen bitácora y conservan un próximo seguimiento activo."
    },
    VENCIDOS: {
      title: "Seguimientos vencidos",
      copy: "Aquí se muestran tanto primeros seguimientos como próximos seguimientos ya vencidos según la fecha actual del sistema."
    },
    PROSPECTOS_GC: {
      title: "Lista de prospectos para Grupos de Conexión",
      copy: "Bienvenida ya pasó estos casos al flujo de grupos, pero el líder aún no concluye el registro dentro del grupo."
    }
  })[activeScope] || {
    title: "Listado operativo de Bienvenida",
    copy: "Aquí conviven los nuevos por atender y los casos que ya van caminando entre Bienvenida y Grupos de Conexión."
  ...
  return `
    <section class="view-grid">
      ${renderModuleMobileHero_({
        ...
        badge: {
          label: activeScope === "SIN_SEGUIMIENTO"
            ? `${withoutFollowupCount} primeros seguimientos`
            : (activeScope === "ALL"
        ...
        <article class="detail-card module-section-anchor welcome-followup-list-card" id="welcome-followup-list">
          ...
          <div class="panel-head" style="margin-top: 18px;">
            <div>
              ...
              <p>Desde aquí puedes registrar el primer seguimiento, controlar el próximo seguimiento y abrir el historial completo sin perder el contexto del caso.</p>
            </div>
            <span class="pill dark">${escapeHtml(String(rows.length))} resultados</span>
          </div>
          ...
          <div class="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Congregante</th>
                  <th>${escapeHtml(followupColumn.label)}</th>
...
function getWelcomeFollowupColumnConfig_(rows, activeScope) {
  const scope = String(activeScope || "ALL").toUpperCase();

  if (scope === "SIN_SEGUIMIENTO") {
    return {
      mode: "first",
  ...
  if (scope === "CON_SEGUIMIENTO" || scope === "VENCIDOS") {
    return {
      mode: "next",
...
function getSelectedWelcomeFollowupRecord_(profile) {
  const followups = Array.isArray(profile?.followups) ? profile.followups : [];
  const selectedId = String(state.ui.selectedWelcomeFollowupId || "");
...
function renderWelcomeFollowupWorkbench_(selectedPerson, selectedProfile, selectedHealth, mode) {
    ...
    return `
      <div class="panel-head">
        <div>
          ...
          <p>Usa las acciones del listado para registrar el siguiente contacto o abrir el historial detallado del asistente.</p>
        </div>
  ...
  const workflowButtonLabel = String(selectedPerson?.welcomeStatus || "").toUpperCase() === "PROSPECTO GP" || selectedPerson?.prospectWorkflowStarted
    ? "Gestionar PGC"
    : "Prospectar GC";
      ...
      ${followups.length ? `
        ...
        <div class="welcome-followup-browser welcome-followup-browser-executive">
          ...
          <div class="welcome-followup-history-detail welcome-followup-history-card">
            ...
            ` : `
              <div class="empty-state">Selecciona un seguimiento para ver su detalle.</div>
            `}
          </div>
        </div>
      ` : `
        <div class="empty-state" style="margin-top: 18px;">Todavía no hay seguimientos registrados para esta persona.</div>
      `}
    `;
  }
...
function renderWelcomeProspectsView_() {
  ...
  const allRows = getWelcomeProspectPeople_({
    includeRegistered: true
  });
  const canDeleteScrap = canUseScrapDelete_();
  const readyCount = rows.filter((row) => getWelcomeProspectWorkflowState_(row).stage === "READY").length;
  const pendingCount = rows.filter((row) => getWelcomeProspectWorkflowState_(row).stage === "PENDING").length;
  const registeredCount = allRows.filter((row) => getWelcomeProspectWorkflowState_(row).stage === "REGISTERED").length;
  const withGroupCount = rows.filter((row) => row.suggestedGroupId).length;
  const feedbackCount = rows.filter((row) => Number(row.leaderFeedbackCount || 0) > 0).length;
  ...
  return `
    <section class="view-grid">
      ${renderModuleMobileHero_({
        ...
        copy: "Aquí ves solamente los casos que todavía siguen pendientes de registro en grupo, conviertes a PGC y mantienes la retroalimentación interna con el liderazgo.",
        badge: {
          label: rows.length ? `${pendingCount} pendientes por registrar` : "Sin pendientes",
          kind: pendingCount ? "warning" : "success"
        },
        metrics: [
          { label: "Listos", value: String(readyCount) },
          { label: "Pendientes", value: String(pendingCount) },
          { label: "Canal interno", value: String(feedbackCount) },
      ...
      <div class="stats-grid assistants-stats-grid">
        <article class="stat-card">
          <span class="status-chip warning">Listos para Prospecto GC</span>
          <strong>${escapeHtml(String(readyCount))}</strong>
          <span>Nuevos que todavía no han sido enviados al flujo de Grupos de Conexión.</span>
        </article>
        <article class="stat-card">
          <span class="status-chip neutral">Pendiente por registrar</span>
          <strong>${escapeHtml(String(pendingCount))}</strong>
          <span>Prospectos GC ya avisados al líder y todavía no registrados dentro del grupo.</span>
        </article>
        <article class="stat-card">
          <span class="status-chip success">Registrados en grupo</span>
          <strong>${escapeHtml(String(registeredCount))}</strong>
          <span>Ya no aparecen en este listado porque el líder completó su registro en grupo.</span>
        </article>
        <article class="stat-card">
          <span class="status-chip neutral">WhatsApp interno</span>
          <strong>${escapeHtml(String(feedbackCount))}</strong>
          <span>Casos donde Bienvenida ya dejó al menos una retroalimentación al líder.</span>
        </article>
      </div>
      ...
      <article class="detail-card">
        <div class="panel-head">
          <div>
            ...
            <p>Se muestran solo los casos que siguen pendientes por registrar en grupo. Desde aquí puedes convertirlos a PGC o abrir el canal interno con el líder.</p>
          </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Aviso al líder</th>
                <th>Acciones</th>
              </tr>
            </thead>
            <tbody>
              ${rows.length ? rows.map((row) => {
                const workflow = getWelcomeProspectWorkflowState_(row);
                const canSendLeaderWhatsapp = workflow.stage === "PENDING" && getWelcomeLeaderWhatsappTargets_(row).length > 0;
                const feedbackMeta = Number(row.leaderFeedbackCount || 0) > 0
                  ? `WhatsApp interno: ${formatDate(row.lastLeaderFeedbackAt) || "Sin fecha"}${row.lastLeaderFeedbackBy ? ` · ${row.lastLeaderFeedbackBy}` : ""}`
                  : "Sin retroalimentación interna todavía";

                return `
                  <tr class="${escapeHtml(workflow.rowClass)}">
                    <td>
                      <span class="row-title">${escapeHtml(row.nombreCompleto || row.nombre || "Sin nombre")}</span>
                    ...
                    <td>
                      ...
                      <span class="row-meta">${escapeHtml(workflow.detail)}</span>
                    </td>
                    <td>
                      <span class="row-title">${escapeHtml(formatDate(workflow.alertAt) || "Sin aviso")}</span>
                      <span class="row-meta">${escapeHtml(workflow.alertStatus || "Telegram pendiente o no requerido todavía")}</span>
                      <span class="row-meta">${escapeHtml(feedbackMeta)}</span>
                    </td>
                    <td>
                      <div class="inline-actions">
                        <button class="btn btn-primary btn-compact" data-action="open-welcome-prospect-modal" data-person-id="${escapeHtml(row.id || "")}">${escapeHtml(workflow.stage === "READY" ? "Convertir a PGC" : "Gestionar")}</button>
                        <button class="btn btn-secondary btn-compact" data-action="open-welcome-prospect-feedback-modal" data-person-id="${escapeHtml(row.id || "")}" ${canSendLeaderWhatsapp ? "" : "disabled"}>WhatsApp líder</button>
                        ${canDeleteScrap ? `<button class="btn btn-danger btn-compact" data-action="prompt-scrap-delete-person" data-person-id="${escapeHtml(row.id || "")}" data-origin-view="welcome-prospects">Eliminar scrap</button>` : ""}
                      </div>
                    </td>
                  </tr>
                `;
                ...
                <tr>
                  <td colspan="5"><div class="empty-state">No hay personas pendientes de registrar en grupo con los filtros actuales.</div></td>
                </tr>
              `}
            </tbody>
          </table>
  ...
  return `
    <section class="view-grid">
      ${renderModuleMobileHero_({
        ...
        metrics: [
          { label: "Listos", value: String(summary.encounterCandidates) },
      ...
      <div class="stats-grid assistants-stats-grid">
        <article class="stat-card">
          <span class="status-chip warning">Listos para invitar</span>
          <strong>${escapeHtml(String(summary.encounterCandidates))}</strong>
          <span>Ya cumplieron la regla de 3 asistencias consecutivas.</span>
        </article>
        <article class="stat-card">
          <span class="status-chip neutral">Invitación enviada</span>
          <strong>${escapeHtml(String(summary.invited))}</strong>
          <span>Están esperando confirmación para registrarse a Encuentro.</span>
        </article>
        <article class="stat-card">
          <span class="status-chip dark">Prospectos GF</span>
...
function calculateDashboardGlobalPercent_(value, total) {
  ...
  return Math.round((Number(value || 0) / safeTotal) * 1000) / 10;
}

function formatDashboardGlobalPercent_(value, total) {
  const percent = calculateDashboardGlobalPercent_(value, total);
  ...
  return [
    {
      ...
      groupNames: ["switch on", "exteens", "fearless", "unlimited"]
    },
    {
      id: "marriages",
      label: "Matrimonios",
      description: "Inseparables",
      groupNames: ["inseparables"]
    },
    {
      id: "menWomen",
      label: "Hombres y Mujeres",
      description: "Entre Mujeres y Hombres de Valor",
  ...
  try {
    ...
    if (action === "promote-welcome-pgc") {
      ...
      state.ui.welcomeModal = {
        ...
        suggestedAt: formatDateForInput_(person?.prospectAlertAt || new Date()) || "",
        notes: String(person?.lastFollowupNotes || person?.notasBienvenida || "")
      };
      renderApp();
      scrollToSection_("app-top");
      return;
    }

    if (action === "select-welcome-followup") {
      state.ui.selectedWelcomeFollowupId = String(button.dataset.followupId || "");
      renderApp();
      return;
    }
    ...
    if (action === "open-welcome-prospect-feedback-modal") {
      const personId = String(button.dataset.personId || "");
      const person = getWelcomePersonById_(personId);
      const workflow = getWelcomeProspectWorkflowState_(person);
      ...
      if (workflow.stage === "READY") {
        showToast("Primero convierte a PGC", "Antes del WhatsApp interno asigna el grupo y guarda el pase a Prospecto GC.", "warning");
        return;
      }
      ...
      state.ui.welcomeModal = {
        ...
        personId,
        actionDate: formatDateForInput_(new Date()) || "",
        notes: "",
        targetIndex: 0
      };
      renderApp();
      ...
      void loadWelcomeProfile_(personId, {
        ...
        if (state.ui.welcomeModal?.kind === "prospect-feedback" && String(state.ui.welcomeModal?.personId || "") === personId) {
          renderApp();
        }
      }).catch(() => {});
      return;
    ...
    if (action === "save-and-send-welcome-prospect") {
      await saveWelcomeProspectAssignment_({
        ...
        loadingMessage: "Guardando prospecto, avisando por Telegram y preparando WhatsApp...",
...
async function handleSubmit(event) {
  ...
  try {
    ...
    if (form.id === "welcome-prospect-feedback-form") {
      await saveWelcomeProspectLeaderFeedback_();
      return;
    }
...
async function handleChange(event) {
  ...
  try {
    ...
    if (target.id === "welcome-prospect-feedback-date") {
      if (state.ui.welcomeModal?.kind === "prospect-feedback") {
        state.ui.welcomeModal.actionDate = target.value;
      }
      return;
    }

    if (target.id === "welcome-prospect-feedback-target") {
      if (state.ui.welcomeModal?.kind === "prospect-feedback") {
        state.ui.welcomeModal.targetIndex = Number(target.value || 0);
        renderApp();
      }
      return;
    }
...
function handleInput(event) {
  ...
  if (target.id === "welcome-prospect-feedback-notes") {
    if (state.ui.welcomeModal?.kind === "prospect-feedback") {
      state.ui.welcomeModal.notes = target.value;
    }
    return;
...
function buildWelcomeFallbackFromDirectory_(person) {
  ...
  return {
    ...
    followupsCount: 0,
    followupTimelineCount: 0,
    prospectWorkflowStarted: false,
    prospectAlertAt: "",
    prospectAlertStatus: "",
    prospectAlertDetail: "",
    prospectPendingGroupFollowup: false,
...
async function loadWelcomeProfile_(personId, options = {}) {
  ...
  const task = () => runSharedLoad_(`welcomeProfile::${cleanPersonId}`, async () => {
    ...
    if (!Array.isArray(state.welcomeProfile?.followups) || !state.welcomeProfile.followups.length) {
      state.ui.selectedWelcomeFollowupId = "";
    } else if (!state.welcomeProfile.followups.some((item) => String(item.id || "") === String(state.ui.selectedWelcomeFollowupId || ""))) {
      state.ui.selectedWelcomeFollowupId = String(state.welcomeProfile.followups[0]?.id || "");
    }
    return state.welcomeProfile;
  });
...
function buildWelcomePersonClientDto_(person) {
  ...
  return {
    ...
    followupTimelineCount: Number(person?.followupTimelineCount || person?.followupsCount || 0),
    prospectWorkflowStarted: Boolean(person?.prospectWorkflowStarted),
    prospectAlertAt: String(person?.prospectAlertAt || "").trim(),
    prospectAlertStatus: String(person?.prospectAlertStatus || "").trim(),
    prospectAlertDetail: String(person?.prospectAlertDetail || "").trim(),
    prospectPendingGroupFollowup: Boolean(person?.prospectPendingGroupFollowup),
...
function buildOptimisticWelcomeProfileAfterFollowupSave_(response, payload) {
  ...
  const isNonPastoralAction = isWelcomeNonPastoralActionType_(actionType);
  const pastoralCountFromTimeline = followups.filter((item) => {
    return !isWelcomeNonPastoralActionType_(item?.actionType);
  }).length;
  const nextFollowupsCount = Math.max(
    Number(sourcePerson.followupsCount || 0),
    pastoralCountFromTimeline,
    Number(previousPerson.followupsCount || 0) + (isNonPastoralAction ? 0 : 1)
  );
  ...
  const nextLeaderFeedbackCount = Math.max(
    Number(sourcePerson.leaderFeedbackCount || 0),
    Number(previousPerson.leaderFeedbackCount || 0) + (actionType === "WHATSAPP_LIDER_GC" ? 1 : 0)
  );
  ...
  return {
    ...
    person: {
      ...
      lastContactAt: String(
        isNonPastoralAction
          ? (sourcePerson.lastContactAt || previousPerson.lastContactAt || "")
          : (savedFollowup?.actionDate || sourcePerson.lastContactAt || previousPerson.lastContactAt || "")
      ).trim(),
      lastActionType: String(
        isNonPastoralAction
          ? (sourcePerson.lastActionType || previousPerson.lastActionType || "")
          : (savedFollowup?.actionType || sourcePerson.lastActionType || previousPerson.lastActionType || "")
      ).trim(),
      lastFollowupResult: String(
        isNonPastoralAction
          ? (sourcePerson.lastFollowupResult || previousPerson.lastFollowupResult || "")
          : (savedFollowup?.result || sourcePerson.lastFollowupResult || previousPerson.lastFollowupResult || "")
      ).trim(),
      lastFollowupNotes: String(
        isNonPastoralAction
          ? (sourcePerson.lastFollowupNotes || previousPerson.lastFollowupNotes || "")
          : (savedFollowup?.notes || sourcePerson.lastFollowupNotes || previousPerson.lastFollowupNotes || "")
      ).trim(),
      nextFollowUpDate: String(
        !isNonPastoralAction && savedFollowup?.nextFollowUpDate !== undefined
      ...
      leaderFeedbackCount: nextLeaderFeedbackCount,
      lastLeaderFeedbackAt: String(
        actionType === "WHATSAPP_LIDER_GC"
          ? (savedFollowup?.actionDate || sourcePerson.lastLeaderFeedbackAt || previousPerson.lastLeaderFeedbackAt || "")
          : (sourcePerson.lastLeaderFeedbackAt || previousPerson.lastLeaderFeedbackAt || "")
      ).trim(),
      lastLeaderFeedbackResult: String(
        actionType === "WHATSAPP_LIDER_GC"
          ? (savedFollowup?.result || sourcePerson.lastLeaderFeedbackResult || previousPerson.lastLeaderFeedbackResult || "")
          : (sourcePerson.lastLeaderFeedbackResult || previousPerson.lastLeaderFeedbackResult || "")
      ).trim(),
      lastLeaderFeedbackNotes: String(
        actionType === "WHATSAPP_LIDER_GC"
          ? (savedFollowup?.notes || sourcePerson.lastLeaderFeedbackNotes || previousPerson.lastLeaderFeedbackNotes || "")
          : (sourcePerson.lastLeaderFeedbackNotes || previousPerson.lastLeaderFeedbackNotes || "")
      ).trim(),
      lastLeaderFeedbackBy: String(
        actionType === "WHATSAPP_LIDER_GC"
          ? (savedFollowup?.systemUserName || sourcePerson.lastLeaderFeedbackBy || previousPerson.lastLeaderFeedbackBy || "")
          : (sourcePerson.lastLeaderFeedbackBy || previousPerson.lastLeaderFeedbackBy || "")
      ).trim()
    },
    summary: {
...
async function saveWelcomeProspectAssignment_(options = {}) {
  ...
  const initialTelegramNotification = normalizeTelegramNotification_(response?.notification);
  const initialTelegramSent = telegramNotificationWasSent_(initialTelegramNotification);
  state.ui.welcomeModal = {
    kind: "prospect",
    ...
    notes: payload.notes,
    notification: initialTelegramSent
  ...
  renderApp();
  if (initialTelegramSent) {
    showToast(
      options.successTitle || "Prospecto actualizado",
      options.successMessage || initialTelegramNotification.detail || "El prospecto quedó pendiente por registrar y Telegram avisó automáticamente al líder.",
      "success"
    );
    return true;
  }
...
function normalizeTelegramNotification_(notification) {
  if (!notification || typeof notification !== "object") {
    return null;
  }
  return {
    sent: notification.sent === true || String(notification.status || "").trim().toUpperCase() === "ENVIADO",
...
function telegramNotificationWasSent_(notification) {
  return notification?.sent === true
    || String(notification?.status || "").trim().toUpperCase() === "ENVIADO";
}

function isMissingWelcomeProspectNotifyActionError_(error) {
  const message = String(error?.message || "").trim();
  return message.includes("Unknown action: welcome.followups.notifyProspectLeader");
}

async function notifyWelcomeProspectLeaderInBackground_(response, payload, options = {}) {
  const legacyNotification = normalizeTelegramNotification_(response?.notification);
  if (telegramNotificationWasSent_(legacyNotification)) {
    return {
  ...
  try {
    ...
    const telegramNotification = normalizeTelegramNotification_(notifyResponse?.notification);
    const telegramSent = telegramNotificationWasSent_(telegramNotification);
    ...
    if (isMissingWelcomeProspectNotifyActionError_(error)) {
      if (state.ui.welcomeModal?.kind === "prospect" && String(state.ui.welcomeModal?.personId || "") === String(payload?.personId || "")) {
        state.ui.welcomeModal = {
          ...state.ui.welcomeModal,
          notification: {
            ...
            sentAt: "",
            status: "PENDIENTE",
            detail: "La API publicada aún no incluye el aviso diferido. Si el líder ya recibió Telegram, puedes continuar con el flujo normal."
          }
        };
        renderApp();
      }
      ...
      showToast(
        "API pendiente de actualizar",
        "Tu despliegue actual todavía no incluye el aviso diferido de Telegram. Si el mensaje ya llegó al líder, puedes continuar normalmente.",
        "warning"
      );
      return null;
    }
...
async function savePendingProspectGroupAlert_(personId) {
  ...
  const initialTelegramNotification = normalizeTelegramNotification_(response?.notification);
  if (telegramNotificationWasSent_(initialTelegramNotification)) {
    showToast(
      "Grupo actualizado",
      draft.groupDirty
        ? `El grupo cambió a ${selectedGroupName}. ${initialTelegramNotification.detail || "Telegram ya avisó al nuevo liderazgo."}`
        : (initialTelegramNotification.detail || "Telegram ya avisó al liderazgo de ese grupo."),
      "success"
    );
    renderApp();
    return true;
...
async function saveWelcomeProspectLeaderFeedback_() {
  const modalState = getWelcomeProspectFeedbackModalState_();
  const currentAuditUser = getCurrentSystemUserAudit_();
  let response = null;
  let whatsappOpened = false;
  const actionDate = modalState?.actionDate || formatDateForInput_(new Date());

  if (!modalState || !modalState.person?.id) {
    showToast("Prospecto no disponible", "Abre nuevamente el caso e intenta otra vez.", "warning");
    return false;
  }

  if (!modalState.groupId) {
    showToast("Falta grupo sugerido", "Primero confirma el grupo de conexión antes de usar el WhatsApp interno.", "warning");
    return false;
  }

  if (!modalState.selectedTarget) {
    showToast("Sin líder con WhatsApp", "Configura al menos un teléfono de líder dentro de Catálogos para habilitar este canal.", "warning");
    return false;
  }

  const preparedWindow = prepareLeaderWhatsappWindow_(true);
  const payload = {
    personId: modalState.person.id,
    actionDate,
    actionType: "WHATSAPP_LIDER_GC",
    result: "WHATSAPP_INTERNO_PREPARADO",
    owner: "BIENVENIDA",
    systemUserName: currentAuditUser.name,
    systemUserEmail: currentAuditUser.email,
    nextFollowUpDate: modalState.person.nextFollowUpDate || "",
    status: "PROSPECTO GP",
    suggestedGroupId: modalState.groupId,
    notes: buildWelcomeProspectLeaderFeedbackRecordNote_(
      modalState.person,
      modalState.group?.name || modalState.person.suggestedGroupName || "",
      actionDate,
      modalState.selectedTarget.name,
      modalState.notes
    )
  };
  ...
  await withLoading(async () => {
    response = await apiPost("welcome.followups.save", payload);
    applyWelcomeProfileToLocalState_(response?.profile || null);
    invalidateWelcomeCache_();
    await loadWelcomePeople_({
      force: true,
      showLoading: false
    });
  }, "Guardando retroalimentación interna...");

  whatsappOpened = openLeaderWhatsappWindow_(preparedWindow, modalState.selectedTarget.url);
  state.ui.selectedWelcomePersonId = payload.personId;
  state.ui.selectedWelcomeFollowupId = String(response?.followup?.id || "");
  state.ui.welcomeModal = null;
  renderApp();
  showToast(
    whatsappOpened ? "Canal interno listo" : "Bitácora guardada",
    whatsappOpened
      ? "La retroalimentación quedó registrada y WhatsApp ya se abrió con el mensaje listo para el líder."
      : "La retroalimentación quedó registrada, pero el navegador bloqueó WhatsApp. Intenta nuevamente desde esta misma ficha.",
...
async function saveWelcomeFollowup_(rawPayload) {
  ...
  if (shouldNotifyProspectLeader) {
    ...
    if (telegramNotificationWasSent_(initialTelegramNotification)) {
      showToast(
        "Aviso enviado",
        initialTelegramNotification.detail || "Telegram avisó correctamente al liderazgo del grupo.",
        "success"
      );
      renderApp();
      return false;
...
function getWelcomePastoralFollowupsCount_(person) {
  ...
  const hasPastoralSignal = Boolean(String(person?.lastContactAt || "").trim())
    ...
    || (latestActionType && !isWelcomeNonPastoralActionType_(latestActionType));

  return explicitCount > 0 ? explicitCount : (hasPastoralSignal ? 1 : 0);
}

function isWelcomeNonPastoralActionType_(value) {
  const key = String(value || "").trim().toUpperCase();
  return key === "PROSPECTO_GC" || key === "WHATSAPP_LIDER_GC";
}
...
function getWelcomeFollowupDateSummary_(person) {
  ...
  if (!nextContactDate) {
    return {
      ...
      tone: "warning",
      overdue: false,
      meta: followupsCount > 0
        ? "Todavía no se definió el próximo seguimiento."
        : "Debes definir la fecha del primer seguimiento."
    };
  }
  ...
  if (nextContactDate < today) {
    return {
      ...
      label: formatDate(nextContactDate) || nextContactDate,
      tone: "danger",
      overdue: true,
      meta: followupsCount > 0
        ? "El próximo seguimiento ya venció y no se ha capturado el contacto."
        : "La fecha del primer seguimiento ya venció y la persona sigue sin bitácora."
    };
  }
  ...
  return {
    ...
    date: nextContactDate,
    label: formatDate(nextContactDate) || nextContactDate,
    tone: followupsCount > 0 ? "success" : "neutral",
    overdue: false,
    meta: followupsCount > 0
      ? "Fecha vigente del último próximo seguimiento."
      : "Fecha programada para el primer seguimiento."
  };
}
...
function getWelcomeDisplayedFollowupSummary_(person, requestedMode = "") {
  ...
  if (normalizedMode === "first") {
    return {
      ...
      tone: followupsCount > 0 ? "success" : schedule.tone,
      overdue: followupsCount > 0 ? false : schedule.overdue,
      meta: followupsCount > 0
        ? "El primer seguimiento ya fue capturado; el control continúa con el próximo seguimiento."
        : schedule.meta,
      clickable: followupsCount === 0 && Boolean(schedule.date)
    };
  }
  ...
  if (normalizedMode === "next") {
    if (!followupsCount) {
      return {
        ...
        tone: "neutral",
        overdue: false,
        meta: "Primero debe registrarse el primer seguimiento.",
        clickable: false
      };
    }
    ...
    return {
      ...
      meta: schedule.meta,
      clickable: Boolean(schedule.date)
    };
  }
  ...
  return {
    ...
    overdue: schedule.overdue,
    meta: schedule.meta,
    clickable: Boolean(schedule.date) && followupsCount === 0
  };
}

function isWelcomeFollowupOverdue_(person) {
  const status = String(person?.welcomeStatus || "").toUpperCase();
  return status === "NUEVO" && getWelcomeFollowupDateSummary_(person).overdue;
}
...
function getWelcomeFollowupHealth_(person) {
  ...
  if (person?.prospectRegisteredInGroup) {
    return {
      label: "Registrado en grupo",
      tone: "success",
      overdue: false
    };
  }
  ...
  if (!followupsCount) {
    if (schedule.overdue) {
      return {
        label: "Primer seguimiento vencido",
        tone: "danger",
        overdue: true
      };
    }
...
function getWelcomeFollowupTypeLabel_(value) {
  ...
  if (key === "WHATSAPP_LIDER_GC") {
    return "WhatsApp interno al líder";
  }
...
function getWelcomeInitialFollowupSummary_(person) {
  return getWelcomeDisplayedFollowupSummary_(person, "first");
}

function getWelcomeNextFollowupSummary_(person) {
  return getWelcomeDisplayedFollowupSummary_(person, "next");
}
...
function getWelcomeProspectWorkflowState_(person) {
  if (person?.prospectRegisteredInGroup) {
    return {
      stage: "REGISTERED",
      label: "Registrado en grupo",
      tone: "success",
      rowClass: "welcome-prospect-row-registered",
      detail: person?.assignedGroupName
        ? `Ya quedó registrado en ${person.assignedGroupName}.`
        : "El líder ya lo registró dentro de su grupo.",
      alertAt: person?.prospectAlertAt || "",
      alertStatus: person?.prospectAlertStatus || "",
      alertDetail: person?.prospectAlertDetail || ""
    };
  }

  if (person?.prospectPendingGroupFollowup || String(person?.welcomeStatus || "").toUpperCase() === "PROSPECTO GP") {
    return {
      stage: "PENDING",
      label: "Pendiente seguimiento de Grupos de Conexión",
      tone: "neutral",
      rowClass: "welcome-prospect-row-pending",
      detail: person?.prospectAlertDetail || "Bienvenida ya entregó el caso y está esperando el registro final en el grupo.",
      alertAt: person?.prospectAlertAt || "",
      alertStatus: person?.prospectAlertStatus || "",
      alertDetail: person?.prospectAlertDetail || ""
    };
  ...
  return {
    stage: "READY",
    label: "Listo para Prospecto GC",
    tone: "warning",
    rowClass: "",
    detail: "Elige el grupo ideal y envía el aviso al líder desde esta misma ficha.",
    alertAt: "",
    alertStatus: "",
    alertDetail: ""
  };
...
function renderWelcomePendingRegistrationPill_(person) {
  const workflow = getWelcomeProspectWorkflowState_(person);
  const tone = ["success", "warning", "danger", "dark", "neutral"].includes(String(workflow.tone || ""))
    ? String(workflow.tone || "")
    : "neutral";
  return `<span class="pill ${tone}">${escapeHtml(workflow.label)}</span>`;
}
...
function renderFormationTimelineItem_(label, value, emptyLabel = "Pendiente") {
  return `
    <div class="formation-timeline-item">
...
function renderWorkflowStatusPill_(value) {
  ...
  if (normalized === "INVITACION_ENVIADA") {
    return `<span class="pill neutral">Invitación enviada</span>`;
  }
...
function getWorkflowStatusLabel_(value) {
  ...
  const labels = {
    ...
    "PROSPECTO GF": "Prospecto para Encuentro",
